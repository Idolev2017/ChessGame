...
GAME_MESSAGE getAllMovesBishop(ChessGame* game,Piece* piece,Location* possibleMoves, int* actualSize){
	...
	while(!leftDownEnd || !rightUpEnd || !leftUpEnd || !rightDownEnd){
		...
		leftDownLoc.row -= 1;
		leftDownLoc.col -= 1;
		if(!isLegalLoc(rightUpLoc)) rightUpEnd = true;
		else if(!rightUpEnd && (getPieceOnBoard(game, rightUpLoc) == NULL || getPieceOnBoard(game, rightUpLoc)->color != piece->color)) {
		...
		if(!isLegalLoc(leftUpLoc)) leftUpEnd = true;
		else if(!leftUpEnd && (getPieceOnBoard(game, leftUpLoc) == NULL || getPieceOnBoard(game, leftUpLoc)->color != piece->color)) {
		...
		if(!isLegalLoc(rightDownLoc)) rightDownEnd = true;
		else if(!rightDownEnd && (getPieceOnBoard(game, rightDownLoc) == NULL || getPieceOnBoard(game, rightDownLoc)->color != piece->color)) {
		...
		if(!isLegalLoc(leftDownLoc)) leftDownEnd = true;
		else if(!leftDownEnd && (getPieceOnBoard(game, leftDownLoc) == NULL || getPieceOnBoard(game, leftDownLoc)->color != piece->color)) {
...
GAME_MESSAGE getAllMovesRook(ChessGame* game,Piece* piece,Location* possibleMoves, int* actualSize){
	...
	while(!rightEnd || !leftEnd || !downEnd || !upEnd){
		...
		downLoc.row -= 1;
		upLoc.row += 1;
		if(!isLegalLoc(rightLoc)) rightEnd = true;
		else if(!rightEnd && (getPieceOnBoard(game, rightLoc) == NULL || getPieceOnBoard(game, rightLoc)->color != piece->color)) {
		...
		if(!isLegalLoc(leftLoc)) leftEnd = true;
		else if(!leftEnd && (getPieceOnBoard(game, leftLoc) == NULL || getPieceOnBoard(game, leftLoc)->color != piece->color)) {
		...
		if(!isLegalLoc(downLoc)) downEnd = true;
		else if(!downEnd && (getPieceOnBoard(game, downLoc) == NULL || getPieceOnBoard(game, downLoc)->color != piece->color)) {
		...
		if(!isLegalLoc(upLoc)) upEnd = true;
		else if(!upEnd && (getPieceOnBoard(game, upLoc) == NULL || getPieceOnBoard(game, upLoc)->color != piece->color)) {
...
GAME_MESSAGE movePawn(ChessGame* game,Piece* piece,Location dest, bool toPrint){
	...
	if(((diffCol == 0 && getPieceOnBoard(game,dest) == NULL) && (whiteCanSkip || whiteJumpForward|| blackCanSkip || blackJumpForward)) || (abs(diffCol) == 1 && (whiteJumpForward || blackJumpForward))){
...
GAME_MESSAGE moveRook(ChessGame* game, Piece* piece,Location dest){
	...
	if(equalRows + equalCols == 0) return GAME_INVALID_MOVE;
...
GAME_MESSAGE moveQueen(ChessGame* game, Piece* piece,Location dest){
	GAME_MESSAGE msg;
	GAME_MESSAGE queenMsg;
	if((msg = moveRook(game, piece, dest)) == GAME_FAILED || (queenMsg = moveBishop(game, piece, dest)) == GAME_FAILED){ //Unknown Error
		return GAME_FAILED;
	}
	else if(msg == GAME_SUCCESS || queenMsg == GAME_SUCCESS) return GAME_SUCCESS;
...
GAME_MESSAGE moveAndCapture(ChessGame* game, Location src, Location dest){
	...
	if(msg == PIECE_THREATENED) return PIECE_THREATENED;
	else if(msg == GAME_FAILED) return msg;
	killPiece(game, dest);
